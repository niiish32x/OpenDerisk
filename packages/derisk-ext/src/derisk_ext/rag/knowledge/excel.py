    ...
    def _find_header_row(
        ...
        for h in best_headers:
            ...
            final_headers.append(
                h if h else f"Col_{len(final_headers)}")  # 确保空标题也有一个默认名
            ...
            for sheet_name in workbook.sheetnames:
                ...
                for index, row in df.iterrows():
                    ...
                    for header_name in final_headers:
                        ...
                        if self._source_columns:
                            if header_str in self._source_columns:
                                ...
                                strs.append(
                                    f"{header_str}: {processed_value}")  # content 中包含 header 更好理解
                        else:
                            processed_value = self.parse_document_body(value_str)
                            strs.append(
                                f"{header_str}: {processed_value}")  # content 中包含 header 更好理解
                    ...
                    if not content.strip():
                        logging.debug(
                            f"Skipping empty content document for row {current_metadata['row']} in sheet {sheet_name}")
                        continue
