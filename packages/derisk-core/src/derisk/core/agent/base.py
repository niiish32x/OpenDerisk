...
from .info import AgentCapability, AgentInfo
...
class AgentBase(ABC):
    ...
    async def execute_tool(
        ...
        if not authorized:
            logger.info(
                f"[{self.info.name}] Authorization denied for tool: {tool_name}"
            )
        ...
        try:
            while self._current_step < self.info.max_steps:
                ...
                if decision_type == "response":
                    ...
                    # Doom loop detection: use hash of tool_name+args
                    # to avoid false positives when same tool is
                    # called with different parameters
                    import hashlib as _hl

                    _call_key = _hl.md5(
                        f"{tool_name}:{json.dumps(arguments, sort_keys=True, ensure_ascii=False)}".encode()
                    ).hexdigest()[:16]
                    if _call_key == _last_call_hash:
                        _consecutive_tool_calls[_call_key] = (
                            _consecutive_tool_calls.get(_call_key, 0) + 1
                        )
                    else:
                        _consecutive_tool_calls.pop(_last_call_hash, None)
                        _consecutive_tool_calls[_call_key] = 1
                        _last_call_hash = _call_key
                    if _consecutive_tool_calls[_call_key] >= _doom_loop_threshold:
                        logger.warning(
                            f"[{self.info.name}] Doom loop: tool '{tool_name}' "
                            f"called {_consecutive_tool_calls[_call_key]} "
                            f"consecutive times with same args. Terminating."
                        )
                        yield (
                            f"\n[DOOM_LOOP] Tool '{tool_name}' called "
                            f"{_consecutive_tool_calls[_call_key]} consecutive "
                            f"times with identical args.\n"
                        )
                        self._state = AgentState.FAILED
                        break
            ...
            if self._state == AgentState.COMPLETED:
                yield "\n[Done]"
                logger.info(
                    f"[{self.info.name}] Run completed, steps={self._current_step}"
                )
